...
export class VKApi {
  ...
  public initializeHandlers(): void {
    ...
    this.hearManager.hear(/^\/help$/i, async (context) => {
    ...
    this.hearManager.hear(/^\/start$/i, async (context) => {
      const [vkUser] = await this.vk.api.users.get({
        user_ids: context.senderId.toString(),
      });

      const isExist = await this.userController.isExist(vkUser.id);
    ...
    this.hearManager.hear(/^\/set_group (.+)$/i, async (context) => {
      const user = await this.userController.getByVkId(context.senderId);
      if (user) {
        await context.send({
      ...
      let groupModel = await this.groupController.getById(group.id);
      ...
      if (!groupModel) {
        ...
        const createdSubGroups = await this.subGroupController.createMany(subGroups);
        groupModel = {
          groupId: group.id,
          groupName: group.label,
          subGroups: createdSubGroups,
        };
        groupModel = await this.groupController.create(groupModel);
      }
    ...
    this.hearManager.hear(/^\/change_group (.+)$/i, async (context) => {
      const [, inputGroup] = context.$match;
      const groupNameRegExp = RegExp(`^${inputGroup}$`, 'i');
    ...
    this.hearManager.hear(/^\/change_subgroup$/i, async (context) => {
      const user = await this.userController.getByVkId(context.senderId);
      if (!user) {
      ...
      const group = await this.groupController.getById(user.group.groupId);
    ...
    this.hearManager.hear(/^\/delete$/i, async (context) => {
      await this.userController.remove(context.senderId);
      await context.send({
    ...
    this.hearManager.hear(/^\/me$/i, async (context) => {
      const user = await this.userController.getByVkId(context.senderId);
      if (!user) {
        await context.send({
    ...
    this.hearManager.hear(/^\/test_user$/i, async (context) => {
      const user = await this.userController.getByVkId(context.senderId);
      ...
      const createdGroup = await this.groupController.create(group);
      ...
      const createdUser = await this.userController.create(candidate);
      await context.send({
        message: `Успешно, вот ты: ${JSON.stringify(createdUser, null, '  ')}`,
      });
    });

    this.hearManager.hear(/^\/test_mark$/i, async (context) => {
    ...
    this.hearManager.hear(/^\/test_search_group (.+)$/i, async (context) => {
      const [, inputGroup] = context.$match;
      const groupNameRegExp = RegExp(`^${inputGroup}$`, 'i');
      const searchResponse = await universityFetch.searchByGroup(inputGroup);
      const filteredResponse = searchResponse.data.filter((el) => groupNameRegExp.test(el.label));
    ...
    this.hearManager.hear(/^\/test_search_subgroup (.+)$/i, async (context) => {
      const [, inputGroup] = context.$match;
      const groupNameRegExp = RegExp(`^${inputGroup}$`, 'i');
      const searchResponse = await universityFetch.searchByGroup(inputGroup);
      const filteredResponse = searchResponse.data.filter((el) => groupNameRegExp.test(el.label));
    ...
    this.vk.updates.on('message_event', async (context) => {
      ...
      switch (context.eventPayload.command) {
        case Commands.ADD_USER:
          {
            ...
            await this.userController.create(candidate);

            const previousMessage = await this.vk.api.messages.getByConversationMessageId({
        ...
        case Commands.CHANGE_GROUP:
          {
            const { oldSubGroupName, newSubGroupName, groupUid, subGroupUid } =
              context.eventPayload;

            await this.userController.updateGroup(context.userId, groupUid, subGroupUid);
        ...
        case Commands.CHANGE_SUBGROUP:
          {
            ...
            await this.userController.updateSubGroup(context.userId, subGroupUid);
